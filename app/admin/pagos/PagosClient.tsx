  ...
  return (
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 30, letterSpacing: '-0.02em', marginBottom: 4 }}>Pagos</h1>
          <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
            Hoy: <span style={{ color: 'var(--green)', fontFamily: 'var(--font-mono)' }}>{fmt(totalHoy)}</span>
          </p>
        </div>
      ...
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {pagos.length === 0 && <tr><td colSpan={6} style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-3)' }}>Sin pagos registrados</td></tr>}
              {pagos.map((p: any) => (
                ...
                >
                  ...
                  <td style={{ padding: '0.875rem 1rem', fontSize: 13, fontWeight: 500 }}>
                    {p.clientes?.nombre} {p.clientes?.apellido}
                  </td>
                  <td style={{ padding: '0.875rem 1rem', fontSize: 13, color: 'var(--text-2)' }}>
                    {p.prestamos?.descripcion}
                  </td>
                  <td style={{ padding: '0.875rem 1rem', textAlign: 'center', fontSize: 13, color: 'var(--text-2)' }}>
                    {p.numero_cuota ?? '—'}
                  </td>
                  <td style={{ padding: '0.875rem 1rem' }}>
                    <span style={{ padding: '0.2rem 0.625rem', borderRadius: 20, fontSize: 11, background: 'var(--bg-3)', color: 'var(--text-2)' }}>
                      {p.metodo_pago}
                    </span>
                  </td>
                  <td style={{ padding: '0.875rem 1rem', fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--green)', fontWeight: 600 }}>
                    {fmt(p.monto)}
                  </td>
                </tr>
              ))}
